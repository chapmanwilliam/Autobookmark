...
def remove_time(txt):
    #makes 26/03/2020 09:00:00 to 26/03/2020
    s=re.sub(' (\d{2}\:)+\d{2}$','',txt)
    return s
def getAge(dt, dob):
    #returns the difference in years between two datetimes
    difference_in_years = (dt - dob).days /365
...
def makedateUniform(str):
    #TODO need to be more sophisticated e.g. to convert 1 May 2020 to 01/05/2020
    ...
    str = re.sub(r'\b(\d{4})\b', replace_function2, str)
    return str
...
def getdate(txt):
    ...
    def getTm(txt):
        # takes 09:10:11:90 and returns hour, minute, seconds, milliseconds
        x = txt.split(":")
        h = 0
        m = 0
        s = 0
