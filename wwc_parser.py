...
#from dateutil.parser import relativedelta #python-dateutil v2.8.1
...
class parserinfo(object):
    ...
    def validate(self, res):
        ...
        if res.year is None or res.month is None: #only allows valid dates thave at least: the year AND the month
            return False
        if res.year is not None and (res.year<1066 or res.year>3000):
            return False
        if res.month is not None and (res.month<1 or res.month>12):
            return False
        if res.day is not None and (res.day<1 or res.day>31):
            return False
        if res.hour is not None and (res.hour<0 or res.hour>23):
            return False
        if res.minute is not None and (res.minute<0 or res.minute>59):
            return False
        if res.second is not None and (res.second<0 or res.second>59):
            return False
    ...
    def validate_skipped_tokens(self,skipped_tokens):
        #returns False if length of any token is greater than MAX_LEN
        #return True
        MAX_LEN=10
        count=0
        for each_token in skipped_tokens:
            if count>0:
                if len(each_token)>MAX_LEN: return False
            count=+1
        return True
    ...
    def get_date_text(self,res):
        ...
        if res.hour is not None:
            h=str(res.hour)
        if res.minute is not None:
            mn=str(res.minute)
        if res.second is not None:
            s=str(res.second)
        #the date
        if d!="":
            dt=d+"/"+m+"/"+y #where we have everything including the day
        else:
            dt=month_text[res.month-1]+" "+ y #where we only have the month and year
            return ", "+ dt
        #time
        tm=""
        if h!="" and mn!="" and s!="":
            b="{:0>2}:{:0>2}:{:0>2}".format(h,mn,s)
            tm=" "+b
        if h!="" and mn!="" and s=="":
            b="{:0>2}:{:0>2}".format(h,mn)
            tm=" "+b
        r=", " + dt + tm
        return r
...
class parser(object):
    ...
    def parse(self, timestr, default=None, ignoretz=False, tzinfos=None, **kwargs):
        ...
        txt=tidy_generally(timestr)
        res, skipped_tokens, dt_text = self._parse(txt, **kwargs)
        #print "skipped tokens: ", skipped_tokens
        #print "removed skipped tokens: ", remove_skipped_tokens(skipped_tokens, txt)
        ...
        try:
            ret = default.replace(**repl) #replaces found values into the default date object and assign to ret
        except:
            return None, None
    ...
    def _parse(self, timestr, dayfirst=None, yearfirst=None, fuzzy=False,
        ...
        #print year, month, day, res.hour, res.minute, res.second
        if not info.validate(res) or not info.validate_skipped_tokens(skipped_tokens):
            return None, None, None

        dt_text=info.get_date_text(res)
        #print dt_text

        if fuzzy_with_tokens:
            return res, tuple(skipped_tokens), dt_text
        else:
            return res, tuple(skipped_tokens), dt_text
...
def tidy_generally(txt):
    ...
    txt=re.sub(r'(\d) *: *(\d)',r"\1:\2",txt) #remove space(s) before and after colon and number
    return txt

def remove_skipped_tokens(skipped_tokens, text):
    if skipped_tokens is None:
        return text
    txt=text
    for each_token in skipped_tokens:
        t=each_token.encode('ascii')
        #print "Remove, ", each_token + "from, ", txt
        if each_token:
            txt=re.sub(t,"",txt,1) #remove t first instance
    return txt
